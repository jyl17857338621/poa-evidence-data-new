...
def salsa20_8(inp, use_const):
    x = list(struct.unpack('<16I', inp))
    orig = x[:]
    if use_const:
        sig = [0x61707865, 0x3320646e, 0x79622d32, 0x6b206574]
        x[0] ^= sig[0]; x[5] ^= sig[1]; x[10] ^= sig[2]; x[15] ^= sig[3]
    for _ in range(4):
...
for const in (False, True):
    for off in (128*8-64, 64):
        print(f"=== use_const={const} off={off} ===")
        for pw, salt, N, r, exp in vecs:
            got = scrypt(pw.encode(), salt.encode(), N, r, 1, 64, off).hex()
            print(f"  N={N} r={r} match={got==exp}")
